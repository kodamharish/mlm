...
class User(models.Model):
    ...
    updated_at = models.DateTimeField(auto_now=True)
    def save(self, *args, **kwargs):
        new_user = not self.pk  # Check if the user is being created

        if self.pk:  # If user exists, check for updated files
            existing_user = User.objects.get(pk=self.pk)
        ...
        super().save(*args, **kwargs)  # Save to generate user_id if new user

        # Move new files to user folder if uploaded
        self.move_files_to_user_folder()
    ...
    def delete(self, *args, **kwargs):
        ...
        if os.path.exists(user_folder) and os.path.isdir(user_folder):
            ...
            os.rmdir(user_folder)  # Remove the folder after deleting all files

        super().delete(*args, **kwargs)  # Delete user record from database
